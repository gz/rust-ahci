...
pub fn main() {
    ...
    hba.ghc.set(hba.ghc.get() | HOST_RESET);
    let mut tmp = hba.ghc.get();
    while tmp & HOST_RESET != 0 {
        tmp = hba.ghc.get();
        println!("reset...");
    }
    ...
    let mut port = &hba.ports[0];

    println!("hba ptr: {:?}", hba as *const _);
    println!("hba ghc: {:?}", &hba.ghc as *const _);
    println!("port ptr: {:?}", port as *const _);
    println!("port serr: {:?}", &port.serr as *const _);
    println!("port cmd: {:?}", &port.cmd as *const _);
    //println!("port tfd: {:?}", &port.tfd as *const _);

    port.serr.set(port.serr.get());
    port.serr.set(0xffffffff);

    let fis_rec = (port.cmd.get() | 0x1 << 4);
    port.cmd.set(fis_rec);

    //std::thread::sleep(Duration::from_millis(1000));

    while (port.tfd.get() & 0x89) != 0 {
        println!("port[0].tfd: 0b{:b}", port.tfd.get());
